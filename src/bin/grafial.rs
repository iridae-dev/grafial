            ...
            Ok(result) => {
                match cli.output.as_str() {
                    "json" => {
                        #[cfg(feature = "serde")]
                        {
                            match serde_json::to_string_pretty(&format_flow_result(&result)) {
                        ...
                        {
                            eprintln!("JSON output requires 'serde' feature. Use 'summary' or 'debug' instead.");
                            process::exit(1);
                        }
                    }
                    "debug" => {
...
fn print_summary(flow_name: &str, result: &grafial::engine::flow_exec::FlowResult) {
    ...
    if !result.graphs.is_empty() {
        ...
        for (name, graph) in &result.graphs {
            println!("  {}: {} nodes, {} edges", name, graph.nodes().len(), graph.edges().len());
        }
    }
    ...
    if !result.exports.is_empty() {
        ...
        for (alias, graph) in &result.exports {
            println!("  {}: {} nodes, {} edges", alias, graph.nodes().len(), graph.edges().len());
        }
    }
...
#[cfg(feature = "serde")]
fn format_flow_result(result: &grafial::engine::flow_exec::FlowResult) -> HashMap<String, serde_json::Value> {
    use serde_json::json;
