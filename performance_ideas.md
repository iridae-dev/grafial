...
#### 1.1 Reduce Clone Operations in Rule Execution

**Location:** `src/engine/rule_exec.rs:507-533`, `src/engine/graph.rs:1112-1152`

**Problem:** Unnecessary cloning of the entire graph for each rule application:
```rust
// Current (line 507)
pub fn run_rule_for_each_with_globals(
    ...
    let mut work = input.clone();  // ⚠️ Clone entire graph upfront
```

**Solution:** Defer cloning until first mutation using Cow semantics:
```rust
use std::borrow::Cow;
...
pub fn run_rule_for_each_with_globals(
    ...
    let mut work = Cow::Borrowed(input);  // Start with borrowed reference

    // ... in execute_actions, clone only when needed:
    if let Cow::Borrowed(graph) = work {
        work = Cow::Owned(graph.clone());
    }
    work.to_mut().apply_action(...);
}
```

**Impact:** 30-40% reduction in allocations for read-heavy rules, no change in semantics.

**Risk:** Low - Bayesian semantics unchanged, existing tests validate correctness.

---

#### 1.2 Use Interned Strings for Node/Edge Types

**Location:** `src/engine/graph.rs:370, 566` (label/ty fields), throughout codebase

**Problem:** String cloning on every graph operation:
```rust
pub struct NodeData {
    pub label: String,  // ⚠️ Cloned frequently
    // ...
}

pub struct EdgeData {
    pub ty: String,     // ⚠️ Cloned in every pattern match
    // ...
}
```

**Solution:** Use string interning with `Arc<str>` or introduce a type registry:
```rust
use std::sync::Arc;

pub struct NodeData {
    pub label: Arc<str>,  // ✓ Cheap clone (8 bytes)
    // ...
}

// Or use a symbol table for zero-cost comparisons:
#[derive(Copy, Clone, Eq, PartialEq, Hash)]
pub struct TypeId(u32);

pub struct TypeRegistry {
    forward: HashMap<Arc<str>, TypeId>,
...
**Before/After:**
```rust
// Before: O(n) string copy
let ty = edge.ty.clone();  // Allocates ~20 bytes avg

// After: O(1) reference increment
let ty = edge.ty.clone();  // Increments Arc counter (8 bytes)
```

**Impact:** 20-30% reduction in allocations, 10-15% speed improvement in pattern matching.

**Risk:** Low - transparent to Bayesian logic, requires updating constructors.

---

#### 1.3 Pre-allocate HashMap Capacity

**Location:** `src/engine/rule_exec.rs:60-64, 70-84`, `src/engine/flow_exec.rs:112-114`

**Problem:** HashMaps start at default capacity (0) and reallocate multiple times:
```rust
pub struct MatchBindings {
    pub node_vars: HashMap<String, NodeId>,  // ⚠️ Multiple reallocations
...
```rust
impl MatchBindings {
    fn with_capacity(patterns: &[PatternItem]) -> Self {
...
// Usage in rule_exec.rs:523
let mut bindings = MatchBindings::with_capacity(&rule.patterns);
```

**Impact:** 5-10% reduction in allocation overhead for multi-pattern rules.

**Risk:** None - pure optimization, no semantic change.

---

#### 1.4 Use `&str` Instead of `String` in Hot Paths

**Location:** `src/engine/graph.rs:626-632, 1441-1474`, `src/engine/rule_exec.rs:571-581`

**Problem:** String cloning in lookup keys:
```rust
pub fn get_edges(&self, node: NodeId, edge_type: &str) -> &[EdgeId] {
    if let Some(&(start, end)) = self.ranges.get(&(node, edge_type.to_string())) {
        // ⚠️ Allocates String on every lookup (hot path!)
```

**Solution:** Change HashMap keys to avoid allocation:
```rust
// Change signature
pub struct AdjacencyIndex {
    ranges: HashMap<(NodeId, Arc<str>), (usize, usize)>,  // Use Arc<str>
    edge_ids: Vec<EdgeId>,
}

// Or use borrowed key lookup (avoids allocation):
use std::borrow::Borrow;

pub fn get_edges(&self, node: NodeId, edge_type: &str) -> &[EdgeId] {
    // Use Borrow trait to lookup with &str instead of String
    let key = &(node, edge_type) as &(dyn Borrow<(NodeId, str)>);
    // ... requires custom Borrow impl
}
```

**Simpler immediate fix:**
```rust
// Store edge_type as Arc<str> in EdgeData (see 1.2)
// Then lookup becomes:
pub fn get_edges(&self, node: NodeId, edge_type: &Arc<str>) -> &[EdgeId] {
    if let Some(&(start, end)) = self.ranges.get(&(node, edge_type.clone())) {
        // Arc clone is cheap (just reference count increment)
```

**Impact:** 15-20% improvement in adjacency queries (called in every pattern match).

**Risk:** Low - requires coordinating with string interning (1.2).

---

#### 1.5 Optimize Delta-Aware Degree Computation

**Location:** `src/engine/graph.rs:1207-1293`

**Problem:** Complex linear scan through deltas with multiple nested conditions:
```rust
pub fn degree_outgoing(&self, node: NodeId, min_prob: f64) -> usize {
    let mut count = 0;

    // Check base edges (O(E) scan)
    for e in &self.inner.edges {
        if e.src == node {  // No index, scans all edges
            // ...
        }
    }

    // Check delta edges (another O(D) scan)
    for change in &self.delta {
        // Complex logic to handle modifications...
    }
}
```

**Solution:** Build a lightweight index or use adjacency structure:
```rust
pub fn degree_outgoing(&self, node: NodeId, min_prob: f64) -> usize {
    // If adjacency is built, use it (O(1) lookup + O(neighbors) scan)
    if let Some(adj) = &self.inner.adjacency {
        return adj.get_all_edges(node)
            .iter()
            .filter(|&&eid| {
                self.edge(eid)
                    .and_then(|e| e.exist.mean_probability(&self.inner.competing_groups).ok())
                    .map(|p| p >= min_prob)
                    .unwrap_or(false)
            })
            .count();
    ...
    // Fallback to current implementation (rare case)
    // ...
}
```

**Impact:** 50-70% improvement in degree computation for large graphs.

**Risk:** Low - requires adjacency index to be built (already exists, line 1412).

---

### 2. Medium Effort (High Impact, Moderate Effort)

#### 2.1 Implement Copy-on-Write Delta Compression

**Location:** `src/engine/graph.rs:682-700, 843-894`

**Problem:** Delta tracking creates many small allocations and clones full nodes/edges:
```rust
pub enum GraphDelta {
    NodeChange { id: NodeId, node: NodeData },  // ⚠️ Full NodeData clone
    EdgeChange { id: EdgeId, edge: EdgeData },  // ⚠️ Full EdgeData clone
    // ...
}

// Every mutation clones:
pub fn set_expectation(&mut self, node: NodeId, attr: &str, value: f64) {
    let mut n = self.node(node).ok_or(...)?.clone();  // ⚠️ Clone entire node
    // ...
    self.delta.push(GraphDelta::NodeChange { id: node, node: n });
}
```

**Solution:** Use fine-grained deltas that only store changed values:
```rust
pub enum GraphDelta {
    NodeAttributeChange {
        node: NodeId,
    ...
    // Keep full variants only for structural changes:
    NodeInserted { node: NodeData },
    EdgeInserted { edge: EdgeData },
    // ...
}

pub fn set_expectation(&mut self, node: NodeId, attr: &str, value: f64) {
    let current = self.node(node).ok_or(...)?;
    let old_posterior = current.attrs.get(attr).ok_or(...)?;

    // Only store the delta, not the full node
    self.delta.push(GraphDelta::NodeAttributeChange {
        ...
        new_mean: value,
        new_precision: old_posterior.precision,  // Unchanged
    });
}
```

**Impact:** 40-60% reduction in memory usage for delta, 25-35% faster apply_delta.

**Risk:** Medium - requires careful implementation to maintain Bayesian correctness, thorough testing needed.

**Trade-off:** More complex delta application logic vs. memory savings.
...
#### 2.4 Implement Query Plan Caching with Better Selectivity Estimation

**Location:** `src/engine/query_plan.rs`

**Problem:** Query plan cache exists but may have suboptimal join ordering:
```rust
pub struct QueryPlanCache {
    cache: HashMap<Vec<PatternItem>, Arc<QueryPlan>>,  // Basic cache
}

impl QueryPlanCache {
    pub fn get_or_create(&mut self, patterns: &[PatternItem], graph: &BeliefGraph) -> Arc<QueryPlan> {
...
**Solution:** Add cardinality estimation and cost-based optimization:
```rust
pub struct QueryPlanCache {
    cache: HashMap<PatternKey, Arc<QueryPlan>>,
    statistics: GraphStatistics,  // Track edge type cardinalities
}

pub struct GraphStatistics {
    edge_type_counts: HashMap<Arc<str>, usize>,
    node_label_counts: HashMap<Arc<str>, usize>,
    // Lightweight statistics updated on graph modifications
}

impl QueryPlanCache {
    pub fn get_or_create(&mut self, patterns: &[PatternItem], stats: &GraphStatistics) -> Arc<QueryPlan> {
        // Cost-based join ordering:
        // 1. Estimate cardinality for each pattern
        let costs: Vec<_> = patterns.iter()
            .map(|p| (p, estimate_cardinality(p, stats)))
            .collect();

        // 2. Order by selectivity (most selective first)
        costs.sort_by_key(|(_, cost)| *cost);

        // 3. Build join tree bottom-up
        QueryPlan {
            ordered_patterns: costs.iter().map(|(p, _)| p).collect(),
            estimated_cost: costs.iter().map(|(_, c)| c).sum(),
        }
    }
}

fn estimate_cardinality(pattern: &PatternItem, stats: &GraphStatistics) -> usize {
    let edge_count = stats.edge_type_counts.get(&pattern.edge.ty).copied().unwrap_or(0);
    let src_count = stats.node_label_counts.get(&pattern.src.label).copied().unwrap_or(1);
    let dst_count = stats.node_label_counts.get(&pattern.dst.label).copied().unwrap_or(1);

    // Simple selectivity estimate: |E| / sqrt(|V_src| * |V_dst|)
    edge_count / ((src_count * dst_count) as f64).sqrt() as usize
}
```

**Impact:** 30-50% improvement for complex multi-pattern rules (3+ patterns).

**Risk:** Low - purely additive optimization, doesn't change semantics.

**Trade-off:** Increased complexity vs. query performance.

---

#### 2.5 Use FxHashMap for Integer Keys

**Location:** Throughout codebase - all `HashMap<NodeId, ...>` and `HashMap<EdgeId, ...>`

**Problem:** Default HashMap uses SipHash (cryptographic hash) which is overkill for integer keys:
```rust
pub(crate) struct BeliefGraphInner {
    pub(crate) node_index: HashMap<NodeId, usize>,  // ⚠️ Slow hash for small keys
    pub(crate) edge_index: HashMap<EdgeId, usize>,
    // ...
}
```

**Solution:** Use `rustc_hash::FxHashMap` for integer keys:
```rust
use rustc_hash::FxHashMap;

pub(crate) struct BeliefGraphInner {
    pub(crate) node_index: FxHashMap<NodeId, usize>,  // ✓ Fast non-cryptographic hash
    pub(crate) edge_index: FxHashMap<EdgeId, usize>,
    // ...
}

// Apply throughout:
// - MatchBindings (rule_exec.rs:60-64)
// - FlowResult (flow_exec.rs:47-58)
// - All internal indexes
```

**Impact:** 10-20% improvement in lookup-heavy operations (pattern matching, evidence application).

**Risk:** Very low - FxHash is production-proven in Rust compiler.

**Trade-off:** Adds dependency (`rustc-hash` crate) vs. performance gains.
...
#### 6.1 Eliminate Intermediate Allocations in Iterator Chains

**Location:** `src/engine/rule_exec.rs:576-580, 550-556`

**Problem:** Collecting into Vec before sorting:
```rust
let mut candidates: Vec<_> = edges.iter()
    .filter(|e| e.ty == edge_type)
    .collect();  // ⚠️ Heap allocation
candidates.sort_by_key(|e| e.id);
```

**Solution:** Use iterator adapters to avoid intermediate collection:
```rust
// For small collections, sort in-place is fine
// For large collections, use streaming approach:
let candidates = edges.iter()
    .filter(|e| e.ty == edge_type)
    .collect::<Vec<_>>();  // Still needed for sort
candidates.sort_unstable_by_key(|e| e.id);  // Use unstable sort (faster)
```

**Better solution when you don't need ownership:**
```rust
// If edges is already sorted by ID, use binary search + scan
let start = edges.binary_search_by_key(&EdgeId(0), |e| e.id).unwrap_or_else(|i| i);
let candidates = edges[start..]
    .iter()
    .filter(|e| e.ty == edge_type)
    .take_while(|e| /* condition */);
// No collection needed!
```

**Impact:** 10-15% reduction in allocation overhead for pattern matching.

**Risk:** Low - simple iterator optimization.
...
1. **Pre-allocate HashMaps** (1.3) - 4 hours
2. **Use FxHashMap** (2.5) - 4 hours
3. **Reduce clone in rule execution** (1.1) - 1 day
4. **Optimize delta degree computation** (1.5) - 1 day
5. **Eliminate string allocations** (1.4) - 1 day

**Expected gain:** 20-30% overall improvement, 40% allocation reduction

### Phase 2: Medium Effort (Week 2-3)
1. **String interning** (1.2) - 2-3 days
2. **Fine-grained delta compression** (2.1) - 3-4 days
3. **Query plan caching improvements** (2.4) - 2 days
4. **Iterator optimizations** (6.1) - 1 day

**Expected gain:** Additional 30-40% improvement, 50% memory reduction
