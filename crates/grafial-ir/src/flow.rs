...
impl From<&grafial_frontend::ast::FlowDef> for FlowIR {
    fn from(f: &grafial_frontend::ast::FlowDef) -> Self {
        let graphs = f
            .graphs
            .iter()
            .map(|g| GraphDefIR {
                name: g.name.clone(),
                expr: match &g.expr {
                    grafial_frontend::ast::GraphExpr::FromEvidence { evidence } => {
                        GraphExprIR::FromEvidence(evidence.clone())
                    }
                    grafial_frontend::ast::GraphExpr::FromGraph { alias } => {
                        GraphExprIR::FromGraph(alias.clone())
                    }
                    grafial_frontend::ast::GraphExpr::Pipeline { start, transforms } => {
                        let ts = transforms
                            .iter()
                            .map(|t| match t {
                                grafial_frontend::ast::Transform::ApplyRule { rule } => {
                                    TransformIR::ApplyRule {
                                        rule: rule.clone(),
                                        mode_override: None,
                                    }
                                }
                                grafial_frontend::ast::Transform::ApplyRuleset { rules } => {
                                    TransformIR::ApplyRuleset {
                                        rules: rules.clone(),
                                    }
                                }
                                grafial_frontend::ast::Transform::Snapshot { name } => {
                                    TransformIR::Snapshot { name: name.clone() }
                                }
                                grafial_frontend::ast::Transform::PruneEdges {
                                    edge_type,
                                    predicate,
