...
///
/// * `Ok(())` - Program is valid
/// * `Err(FrontendError::ValidationError)` - Semantic error with description
///
/// # Example
///
/// ```rust,ignore
...
fn validate_belief_model(model: &BeliefModel, schemas: &[Schema]) -> Result<(), FrontendError> {
    ...
    if let ExprAst::Call { name, args } = expr {
        match name.as_str() {
            "sum_nodes" => {
                ...
                if !has_label {
                    return Err(FrontendError::ValidationError(
                        "sum_nodes: missing 'label' argument".into(),
                    ));
                }
                if !has_contrib {
                    return Err(FrontendError::ValidationError(
                        "sum_nodes: missing 'contrib' argument".into(),
                    ));
                }
            }
            "fold_nodes" => {
                ...
                if !has_label {
                    return Err(FrontendError::ValidationError(
                        "fold_nodes: missing 'label' argument".into(),
                    ));
                }
                if !has_init {
                    return Err(FrontendError::ValidationError(
                        "fold_nodes: missing 'init' argument".into(),
                    ));
                }
                if !has_step {
                    return Err(FrontendError::ValidationError(
                        "fold_nodes: missing 'step' argument".into(),
                    ));
                }
            }
            "count_nodes" => {
                ...
                if !has_label {
                    return Err(FrontendError::ValidationError(
                        "count_nodes: missing 'label' argument".into(),
                    ));
                }
            }
            "avg_degree" => {
                ...
                if !has_label {
                    return Err(FrontendError::ValidationError(
                        "avg_degree: missing 'label' argument".into(),
                    ));
                }
                if !has_edge_type {
                    return Err(FrontendError::ValidationError(
                        "avg_degree: missing 'edge_type' argument".into(),
                    ));
                }
            }
            _ => {}
        ...
        if let ExprAst::Call { name, args } = e {
            if name == "prob" {
                ...
                match pos[0] {
                    ...
                    _ => Err(FrontendError::ValidationError(
                        "prob(): argument must be 'edge' in prune_edges predicate".into(),
                    )),
                }
            } else {
                Ok(())
...
fn validate_expr_in_rule(
    ...
    walk_expr(expr, &mut |e| match e {
        ...
        ExprAst::Call { name, args } if name == "E" => {
            ...
            if !named.map.is_empty() || pos.len() != 1 {
                return Err(FrontendError::ValidationError(
                    "E[]: expected single positional argument".into(),
                ));
            }
            match pos[0] {
                ExprAst::Field { target, field: _ } => match &**target {
                    ...
                    _ => Err(FrontendError::ValidationError(
                        "E[]: must be E[NodeVar.attr]".into(),
                    )),
                },
                _ => Err(FrontendError::ValidationError(
                    "E[]: must be a field access expression".into(),
                )),
            }
        }
        _ => Ok(()),
