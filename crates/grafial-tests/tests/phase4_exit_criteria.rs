...
fn phase4_exit_criteria_complete_flow_execution() {
    ...
    let result = run_flow_with_builder(&program, "Demo", &build_evidence_graph, None)
    ...
    assert_eq!(
        ...
        3,
        "base graph has 3 edges initially"
    );

    // VERIFY 2: Can apply transforms in a pipeline
    let cleaned_graph = result.graphs.get("cleaned").expect("cleaned graph exists");
    assert_eq!(
        cleaned_graph.nodes().len(),
        3,
        "nodes preserved through pipeline"
    );

    // VERIFY 3: Can verify structure - edges pruned correctly
    assert!(
        cleaned_graph.edges().len() < base_graph.edges().len(),
        "edges were pruned"
    );
    assert_eq!(
        cleaned_graph.edges().len(),
        1,
        "only high-probability edge remains"
    );

    // VERIFY 4: Can verify beliefs - check edge probabilities
    let remaining_edge = &cleaned_graph.edges()[0];
    let prob = cleaned_graph
        .prob_mean(remaining_edge.id)
        .expect("can read probability");
    ...
    let expectation = cleaned_graph
        .expectation(NodeId(1), "score")
        .expect("can read expectation");
    ...
    let exported = result
        .exports
        .get("demo_output")
        .expect("exported graph exists");
    assert_eq!(
        exported.edges().len(),
        cleaned_graph.edges().len(),
        "exported graph matches cleaned graph"
    );
