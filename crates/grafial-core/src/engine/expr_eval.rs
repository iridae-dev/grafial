...
pub trait ExprContext {
    ...
    fn eval_function(
        ...
        name: &str,
        pos_args: &[ExprAst],
        all_args: &[grafial_frontend::ast::CallArg],
        graph: &BeliefGraph,
...
pub fn eval_binary_op(op: BinaryOp, left: f64, right: f64) -> Result<f64, ExecError> {
    ...
    if !result.is_finite() {
        return Err(ExecError::ValidationError(format!(
            "binary operation produced non-finite value: {} {:?} {}",
            left, op, right
        )));
    }
...
pub fn eval_expr_core<C: ExprContext>(
    ...
    match expr {
        ...
        ExprAst::Call { name, args } => {
            ...
            let pos_args: Vec<ExprAst> = args
                .iter()
                .filter_map(|a| match a {
                    grafial_frontend::ast::CallArg::Positional(e) => Some(e.clone()),
                    grafial_frontend::ast::CallArg::Named { .. } => None,
                })
                .collect();
        ...
        ExprAst::Exists { .. } => {
            ...
            Err(ExecError::Internal(
                "Exists subqueries must be handled by RuleExprContext".into(),
            ))
        }
    }
}
...
mod tests {
    ...
    impl ExprContext for TestContext {
        ...
        fn eval_function(
            ...
            _name: &str,
            _pos_args: &[ExprAst],
            _all_args: &[grafial_frontend::ast::CallArg],
            _graph: &BeliefGraph,
        ) -> Result<f64, ExecError> {
    ...
    fn eval_expr_core_literals() {
        ...
        assert_eq!(
            eval_expr_core(&ExprAst::Number(42.0), &graph, &ctx).unwrap(),
            42.0
        );
        assert_eq!(
            eval_expr_core(&ExprAst::Bool(true), &graph, &ctx).unwrap(),
            1.0
        );
        assert_eq!(
            eval_expr_core(&ExprAst::Bool(false), &graph, &ctx).unwrap(),
            0.0
        );
    }
    ...
    fn eval_expr_core_variables() {
        ...
        assert_eq!(
            eval_expr_core(&ExprAst::Var("x".into()), &graph, &ctx).unwrap(),
            10.0
        );
