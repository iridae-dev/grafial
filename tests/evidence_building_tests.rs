...
fn build_graph_from_evidence_creates_nodes() {
    ...
    assert!(score > 0.0, "Expected score > 0 after observing 10.0, got {}", score);
}
...
fn build_graph_from_evidence_creates_edges() {
    ...
    assert!(prob > 0.5, "Expected prob > 0.5 after observing present, got {}", prob);
}
...
fn build_graph_from_evidence_handles_absent_edge() {
    ...
    let prob = graph.prob_mean(edge.id).unwrap();
    assert!(prob < 0.5, "Expected prob < 0.5 after observing absent, got {}", prob);
}
