...
fn create_test_graph_with_competing_edges() -> BeliefGraph {
    ...
    graph.competing_groups_mut().insert(CompetingGroupId(1), group);
...
fn rule_context_winner_function() {
    ...
    impl ExprContext for TestContext {
        ...
        fn eval_function(
            ...
            match name {
                "winner" => {
                    ...
                    let nid = *self.bindings.node_vars.get(node_var)
        ...
        fn eval_field(&self, _target: &ExprAst, _field: &str, _graph: &BeliefGraph) -> Result<f64, ExecError> {
            Err(ExecError::Internal("field access not supported in test context".into()))
        }
    }
...
fn dynamic_category_discovery_adds_new_categories_to_existing_group() {
    ...
    assert_eq!(group1.categories.len(), 1, "Group should have 1 category after first edge");
    ...
    assert_eq!(group2.categories.len(), 2, "Group should have 2 categories after dynamic discovery");
    ...
    assert!((total - 1.0).abs() < 1e-10, "Probabilities should sum to 1.0, got {}", total);
    // S2 should have higher probability (observed twice vs S3 once)
    assert!(means[0] > means[1] || means[1] > means[0], "One category should have higher probability");
}
...
fn dynamic_category_discovery_rejects_explicit_prior() {
    ...
    assert!(result.is_err(), "Should reject dynamic category discovery for explicit prior");
    let err_msg = result.unwrap_err().to_string();
    // The error might be about category count mismatch or dynamic discovery
    assert!(err_msg.contains("Dynamic category discovery not supported") || 
            err_msg.contains("must match number of categories") ||
            err_msg.contains("not found"), 
            "Error should mention dynamic discovery or category mismatch, got: {}", err_msg);
}
